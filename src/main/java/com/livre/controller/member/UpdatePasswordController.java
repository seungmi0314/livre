...
public class UpdatePasswordController extends SuperClass {
	 ...
	 * response) throws Exception { super.doGet(request, response); String
	 * memberEmail = request.getParameter("memberEmail");
	 * 
	 * System.out.println(memberEmail); MemberDao dao = new MemberDao(); Member bean
	 * = dao.getDataBean(memberEmail); System.out.println("doget");
	 * System.out.println(bean); super.session.setAttribute("logInfo", bean);
	 * 
	 * // 비밀번호 변경 페이지로 직접 이동 super.goToPage(PREFIX + "updatePassword.jsp"); }
	 */
